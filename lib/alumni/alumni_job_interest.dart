...
class AlumniInterestedUsersPage extends StatefulWidget {
  final String jobId;

  const AlumniInterestedUsersPage({super.key, required this.jobId});

  @override
  State<AlumniInterestedUsersPage> createState() =>
      _AlumniInterestedUsersPageState();
}

class _AlumniInterestedUsersPageState extends State<AlumniInterestedUsersPage> {
  String search = '';
  bool loading = true;
  ...
  Future<void> loadData({bool loadMore = false}) async {
    ...
    for (final d in snap.docs) {
      ...
      if (studentSnap.docs.isNotEmpty) {
        userData = studentSnap.docs.first.data();
      } else {
        // 🔍 CHECK ALUMNI
        ...
        if (alumniSnap.docs.isNotEmpty) {
          userData = alumniSnap.docs.first.data();
        }
      }
